...
const startRun = () => {
    console.log("Starting COPY run...");
    copyStore.startRun.output = "";
    copyStore.startRun.finished = false;
    invoke("run_rclone", {
        ...
        copyStore.startRun.started = true;
    });
};
    ...
    if (copyStore.startRun.started) {
        ...
        invoke("get_logs").then((response) => {
            ...
            if (responseString.length > 0) {
                ...
                copyStore.startRun.output = responseString + "\n" + copyStore.startRun.output;
...
<template>
    <v-container class="h-screen">
        ...
        <v-row class="ma-4">
            <v-btn variant="flat" color="red" @click="startRun()" :disabled="copyStore.startRun.started">Start</v-btn>
            <v-btn variant="flat" color="red" @click="stopRun()" :disabled="!copyStore.startRun.started" class="button-left-margin">Stop</v-btn>
        </v-row>
