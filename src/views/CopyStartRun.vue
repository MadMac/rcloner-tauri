        ...
        invoke("get_logs").then((response) => {
            ...
            if (responseString.length > 0) {
                ...
                copyStore.startRun.output =
                    responseString + "\n" + copyStore.startRun.output;
                if (
                    responseString.includes("Rclone process exited with status")
                ) {
    ...
    <v-container class="h-screen">
        <v-row class="ma-4">
            <v-btn
                class="ma-2"
                variant="flat"
                @click="router.push('/copy/dry-run')"
        ...
        <v-row class="ma-4">
            <v-text-field
                :placeholder="copyStore.buildCopyRunCommand()"
                readonly
            ></v-text-field>
        </v-row>
        <v-row class="ma-4">
            <v-btn
                ...
                color="red"
                @click="startRun()"
                :disabled="copyStore.startRun.started"
            >
                Start
            </v-btn>
            <v-btn
        ...
        <v-row class="ma-4 h-50">
            <v-textarea
                ...
                v-model="copyStore.startRun.output"
                readonly
            ></v-textarea>
        </v-row>
    </v-container>
